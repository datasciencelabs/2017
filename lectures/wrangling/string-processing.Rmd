...
In the Web Scrapping Section we encountered an example related to creating the `murders` dataset we explored in the R chapter. After successfully extracting the raw data from a web page into a table:
...
But be careful you chose the correct single quote since using the backquote will give you an error:
...
So we've learned how to write 5 feet and 10 inches separately but what if we want to write them togeter to represent _5 feet and 10 inches_ like this `5'10"`. In this case neither the single or double quote will work! This

```{r, eval=FALSE}
s <- '5'10"'
```

close the string after 5 and this 

```{r, eval=FALSE}
s <- "5'10""
```
closes the string after 10. Note that if we type one of the above code sniptes into R, it will get stuck waiting for you to close the open quote abd you will have to escape with the _esc_ button.
...
In general, string processing involves a string and a pattern. In R we usually store strings in a character vector such as `murders$population`. Above we scrapped the web to obtain the `murders_raw` data. The first three string in this vector defined by the population variable are:
...
The table below includes the functions available to you in the `strigr` package. We split them by task. We also include the R-base equivalent when available. 
...
Above we used used code to scrape the web and create the `murders_raw` table. We noted that three columns need to be parsed from characters into numbers but that commas where making it hard. We can use the `str_detect` function to see that two of the three columns have commas in the entries:
...
This is case is relatively simple compared to the string processing challanges that we typically face in data science. The next example is a rather complex one.
...
If we try to parse it into number we get a warning:
...
A first step in this type of task is to survey the problematic entries and try to define specific patterns followed by a large groups of entries. The larger these groups the more entries we can fix with a single programmatic approach. We want to find patterns that can be accurately described with a rule: such as "a digit, followed by a feet symbol followed by one or two digites followed by a inches symbol".

To look for such patterns it helps to remove the entries that are consistent with being inches and view only the problematic entries. 
We write a function to automatically do this. We keep entries that either result in `NA`s when applying `as.numeric` or are outside a range of plausible heights. We permit a range that covers about 99.9999% of the adult population. We also use `suppressWarnings` to avoid the warning message we know `as.numeric` will gives us.
...
We can now view all the cases by simply printing them. We don't do that here becuase there are 297, but after surveying them carefully we three patterns that followed by three large groups.
...
Throughout this section you will see that we create strings to test our regex. To do this we define patterns that we know should match and also patterns that we know should not. We will call them `yes` and `no` respectively. This permits us to check for the two type of errors: failing to match and incorrectly matching.


#### Character classes

Character classes are used to define a series of character that can be matched. We define character classes with square brackets `[]`. So, for example, if we want the pattern to match only if we have a `5` or a `6`, we use the regex `[56]`

```{r}
str_view(s, "[56]")
```

Suppose we want to match values between 4 and 7. A common way to define character classes is with ragnes.  So, for example, `[0-9]` is equivalent to `\\d`. The pattern we want is `[4-7]`. 
...
However, it is important to know that in regex everything is a character, there are no numbers. So `4` is the character `4` not the number four. Note, for example, that `[1-20]` does **not** mean 1 through 20, it means the characeters 1 through 2 and the character 0. So `[1-20]` simply means the character class coposed of 0, 1 and 2. 

Note that characters do have an order and the digits do follow the numeric order. So 0 comes before 1 which comes before 2 and so on. For the same reason we can define letters as `[a-z]` are lowercase letters, `[A-Z]` are uppoercase letters and `[a-zA-z]` are both. 

#### Anchors

What if we want a match when we have exactly 1 digit? This will be useful in our case study since feet are never more than 1 digit so a restriction will help us. One way to do this with regex is by using _anchors_ which let us define pattern that must start or end a specific places. The two most common anchors are 
...
With what we have learned we can now construct and example for the pattern `x'y\"` with `x` feet and `y` inches.
...
The pattern is now getting complex, but can look at it carefully and break it down:  
...
match the pattern we defined. To see why this is we show some example that expose why we don't have more matches:
...
Now we can understand the difference between the functions `str_extract` and `str_match`:  `str_extract` extracts only strings that match a patterns, not the values defined by groups:
...
The regex special character for the `i`-th groups is `\\i`. So `\\1` is the value extracted from the first group, `\\2` the value from the second and so on. So as a simple example, note that the following code will replace a comma by period but only if it is between two digits:
...
Now we are ready to define a pattern that helps us convert all the `x.y`, `x,y` and `x y` to our prefered format. We need to adapt `pattern_with_groups` to be bit more flexible and capture all the cases.
...
and will be able to perfrom the seach and replace:
...
Let's see how many of these we can make fit our pattern after we the several processing we developed.
...
We can adapt this code slightly to handle the case 2 as well which covers the entry `5'`. Note `5'` is left untouched. This is because the extra `'` make the pattern not match since we have to end with a 5 or 6. We want to permit the 5 or 6 to be followed by 0 or 1 feet sign. So we can simply add `'{0,1}` after the `'` to do this. However, we can use the none or once special character `?`. As we saw above, this is different from `*` which is none or more. We now see thtat the fourth case is also converted:

```{r}
str_replace(s, "^([56])'?$", "\\1'0")
```

Note that here we only permit 5 and 6 but not 4 and 7. This is because 5 and 6 feet tall is quite common, so we assume those that typed 5 or 6 really meant `60` or `72` inches. However, `4` and `7` feet tall are so rare that, although we accept `84` as a valide entry, we assume `7` was entered in error. 

We can use quantifies to deal with **case 3**. These entries are not matched because the inches include decimals and our pattern does not permit this. We need allow the second group to include decimals not just digits. This means we must permit zero or one period `.` then zero or more digits. So we will be using both `?` and `*`.
Also remember that for this particular case remember that the period needs to be escaped since it is a special character (it means any character excpet line break). Here is a simple example of how we can use `*`. 
...
In general, spaces at start or end of the string are uninformative. 
...
One of the entries writes out numbers as words `Five foot eight inches`. Although not efficient, we could add 12 extra `str_replace` to convert `zero` to `0`, `one` to `1`, and so on. To avoid having to write two separate operations for `Zero` and `zero`, `One` and `one`, etc.. we can use the `str_to_lower` function to make all works lower case first:
...
We are now be ready to define a procedure that 
handle convert all the problematic cases to 

#### Putting it all together

We now put all this together into a function that takes a string vecotr and tries to convert as many strings as possible to one format. We write a function that puts together what we have done above.
...
We can aslo write a function that converts words to numbers:
...
they all seem to be cases that 

### `separate`

In a previous section we constructed a regex that let's us identify which elements of a character vector match the feet and inches pattern. However, we want to do more, we want to extract and save the feet and number values so that we can convert them to inches when appropriate.
...
But we can use `extract`. The regex here is a bit more complicated as we have to permit `'` with spaces and `feet`. We also do not want the `"` included in the value, so we do not included that in the group:
...
Now we can adapt the code above using the new techniques we learned
...
Another very common data wrangling operation is string splitting. To illustrate how this comes up, we start with a illustrative example. Suppose we did not have the function `read_csv` availalbe to use. We instead have to read a csv  file using the base R function `readLines` like this:
...
We want to extract the values that are separated by comma for each string in the vector. The commant `str_split` does exactly this:
...
However, because this is such a common task, purr provides a shortcut: if the second argument, instead of a function receives an integer, it assumes we want that entry:

```{r}
map(x, 1) %>% head()
```

To for map to return a character vector instead of a list, we can use `map_chr`. Similary `map_int` returns integers. So to create our data frame we can use:
...
Using other functions included in the purrr package we can accomplish with more efficient code:
...
It turns out there we can use the function argument `simplify=TRUE` to have `str_split` return a matrix instead of a
...
### Case study: Extracing table from a PDF

One of the datasets provided in dslabs shows scintific funding rates by gender in the Netherlads:
...
```{r, echo=FALSE}
knitr::include_graphics("img/pnas-table-s1.png")
```

We could extract the number by hand, but this coud lead to human error. Instead we can try to wrangle the data using R. We start by downloading the pdf document then importing into R:
...
The steps avoid can actually be skipped because we include this raw data in the dslabs package as well
...
we see that it is a long string and each line on the page, including the table rows are separated by the symbol fo newline: `\n`. We can therefore can create a list the lines of the text as elements:
...
By examining this obejct

```{r}
tab %>% head
```

we see that the information for the colmn names is the third and forth entires:
...
In the table the column information is spread across two lines. We want to create one vector with one name for each column. Using some of the function we have just learned we do this:
...
We want to remove the leading space and the the stuff following the comma. We use regex for the latter. The we can obtain the elements by splitting using the space. We want to split only when there is 2 or more spaces to avoid spliting `success rate`. So we use the regex `\\s{2,}`
...
Now looking that the second line:
...
Now we are ready to get the actual data. By examinin the `tab` object we notice that the information is in lines 6 through 14. We can use `str_split` again to achieve our goal:
...
Another common operation involving string is recoding the names of categorical variables. For example, if you have a really names for your levels and you will be displaying them in plots, you might want to use shorter versions of these names. For example in character vector with country names you might want to change "United States of America" to "USA" and "United Kingdom" to UK, and so on. We can do this with `case_when` but the tidyverse offers options that specifically designed for this task: the `recode` function. 
...
We have four countries with names longer than 12 characters. These names appear once for eacery year in the gapminder dataset and once we pick nicknames we need to change them all consistenly. The `recode` function can be used to do this:
...
Other similar functions `recode_factor` and `fct_recoder` in the forcats tidyverse package.
