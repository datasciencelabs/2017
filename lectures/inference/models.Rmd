...
How was Mr. Silver so confident. We will demonstrate Mr. Silver saw that Mr. Scarborough did not using a Monte Carlo simulation. We generate results for 12 polls taken the week before the election. We mimic sample sizes from actual polls and construct. We construct and report  95\% confidence intervals for each of the 12 polls:
...
Are we know ready to declare a probability of Clinton winning the popular vote? Not yet. In our model $d$ is a fixed parameters so we can't talk about probabilities. To provide probabilities, we will need to learn about Bayesian statistics.
