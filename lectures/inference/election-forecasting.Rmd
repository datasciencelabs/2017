...
X_j = d + \varepsilon_j
$$
we use the index $j$ to represent the different polls and we define $\varepsilon_j$ to be a random variable that explains the poll to poll variability introduced by sampling error. To do this we assume ti has  0 and standard error $2 \sqrt{p(1-p)/N}$. If $d$ is 2.1 and the sample size for these polls was 2,000 we can simulate $J=6$ data points from this model like this:
...
Note that $h_i$ is common to all the observed spreads from a specific pollster. Different pollster have a different $h_i$ which explains why we can see the groups of points shift up and down from pollster to pollster.
...
We are now ready to predict the electoral college result for 2016. We start by aggregating results from poll taken during the last week before the election:
...
Here are the 10 closes races according to the polls:

```{r}
results %>% arrange(abs(avg))
```

We now introduce the command `left_join` that will let us easily add the number of electoral votes for each state from the data set `us_electoral_votes_2016`. We will describe this function in detail in the Wrangling Chapter. Here we simple say that the function combines the two datasets so that the information from the second argument is added to the information in the first:
...
Note that the estimates based on posterior does move the estimates towards 0, although the states with many polls are influenced less. This is expected as the more poll data we collect the more we trust those results:
...
Forecaster like to make predictions well before the election. The predictions are adapted as new polls come out. However, an important questions forecasters must ask is: how informative are polls taken several weeks before the election? Here we study the variability of poll results across time.
...
Since there is not pollster effect, then perhaps the theoretical standard error matches the data-derived standard deviation. We compute both here:
...
This implies that, if we are going to forecast, our model must include a term to model that accounts for the time effect. We would write a model including a bias term for time:
...
Pollster also try to estimate trends, call them $f(t)$, from these data and incorporate these into their predictions. The blue lines in the plots above 
