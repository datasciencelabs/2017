...
To performance statistics used in the Netflix challenge was the residual mean squared error (RMSE). This is related to the MSE error function we described earlier. If \hat{Y} is our prediction and $Y$ is the observed rating, then we prefer recommendation systems that minimize:
...
From looking at the distribution of ratings, we can guess that this is the standard deviation of that distribution. We get a RMSE of about 1. To win the grand prize of $1,000,000, a participating team had to get an RMSE of about 0.857. 
...
We can again use least squared to estimate the $b_i$ but note that there are thousands of $b_i$ since each movie gets one. If we try using the `lm()` function to obtain least square estimates, our computer will probably crash.
...
idea is to minimize the sum of squares equation while penalizing for large values of $b_i$.
...
factorization. The winning team used this approach, as described [here](http://www.netflixprize.com/assets/ProgressPrize2008_BellKor.pdf).
This approach is very much related to PCA described in 
...
For this example we have 5 users and 12 movies. So our residual matrix is $12\times 5$:
...
We can see some _structure_ in this matix. By structure, we mean that the matrix entries do not appear to be independent identically distributed. Some movies are correlated to each other
...
Matrix factorization refers to the fact that we have describe the matrix above by _factorizing_ it like this:
...
The name _matrxi factorization_ comes from the fact that if we use matrix multiplication, we can wrtie 
...
Here is a plot $r_{u,i}$ next to  $\mathbf{p} \mathbf{q}_$:
...
The structure in our movie data seems to be much more complicated than gangster movie versus romance. We may need more factors to explain the variability. For example we may have:
...
The model now has more parameters, 34,  but still less than the original data 60.  
$$
...
with the columns of $\mathbf{P}$ and $\mathbf{Q}$ are uncorrleated and the 
...
```{r}
r[is.na(r)] <- 0
pca <- prcomp(r - rowMeans(r), center=TRUE, scale=FALSE) ## svd is faster here
