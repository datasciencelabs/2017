...
The main problems comes from estimating correlations for 10 predictors. With 10, we have 45 correlations for each class. In general the formula is $p(p-1)/2$ which gets big quickly. 
...
which is the negative of a distance squared scaled by the standard deviation. For higher dimensions the same is true expect the scaling is more complex and involves correlations.
...
We can also compare to KNN with two different values of `k`
...
To summarize these curves into one single number that can be compared across methods, it is common to take the area under the ROC curve (abbreviated AUC). Higher values indicate better performance.
...
We'll create a training set so we can test out how each method does on the three class problem.
