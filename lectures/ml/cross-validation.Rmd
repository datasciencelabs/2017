...
784 is too much to handle in a demo. So we will compress the predictors by combining groups of 16 pixels. 
...
Let's write a function to perform the compression.
...
Now, we compress the entire dataset. This will take a bit of time:
...
Let's remove these "low information" features:

```{r}
keep_columns <- which(column_means>10)
``` 

Let's next define the training data and test data:
