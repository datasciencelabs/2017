...
In these cases it is often convenient to save the predictors in a matrix and the outcome in a vector rather than using a data frame. The main reason is that certain mathematical operations needed to develop efficient code can be performed using techniques from a branch of mathematics called _linear algebra_. In fact, linear algebra and matrix notation are key elements of the languange used in academic papers describing machine learning techniques. We will not cover linear algebra in detail here, but will demonstrate how to use matrices in R so that you can apply the linear algebra techniques already implemented in R base or other packages.

To motivate the use of matrixes we will pose several challenges. 
...
3. Zero out low values. First, look at the distribution of all pixel values, use this to pick a cutoff to define unwritten space. Then make anything below that cutoff 0. 
...
However, we can explicitly convert a vector into a matrix using the function `as.matrix`:
...
We can convert a vector into a matrix with the `matrix` function and specifying the number of rows and columns the resulting matrix should have. The matrix is filled in by column: the first column is filled first, the second column second, and so on. This example helps illustrate:
...
But note that, just like with `sapply` and `map`, can perform any function with `apply`. So if we wanted the population standard deviation for each column we could write
...
We can use numeric indices to extract columns 
...
We can quickly make a histogram of all the values in our dataset. We saw how we can turn vectors into matrix. We can also undo this and turn matrices into vectors. The operation will happen by row:
...
We notice a clear dichotomy which is explained as parts with and without ink. If we think that values below, say, 25 are smudges, we can quickly make them zero using:
