...
Statistics have been used in baseball since its beginnings. The dataset we will be using, included in the `Lahman` library, goes back to the 19th century. For example, a summary statistic we will describe soon, _Batting average_, has been used for decades to summarize a batter's success. [Other statistics](http://mlb.mlb.com/stats/league_leaders.jsp) such as home runs (HR), runs batted in (RBI), and stolen bases (SB) are reported for each player in the game summaries included in the sports section of newspapers, with players rewarded for high numbers.
...
We know that, by definition, HRs cause runs. It could be that HRs also cause BBs and make it appear as if BB cause runs. This is called confounding, an important concept we will learn about.

Linear regression will help us parse all this out and quantify the associations. This will then help us determine what players to recruit. Specifically, we will try to predict things like: _how many more runs will a team score if we increase the number BBs but keep the HRs fixed?_ Regression will help us answer this question.
